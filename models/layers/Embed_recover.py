...
class TemporalFeatureEmbedding(nn.Module):
    ...
    def forward(self, x):
        # x: [batch, Seq_Len, 12] - containing the following columns:
        # hour, quarter_hour, day, day_in_week, hour_sin, hour_cos,
        # quarter_hour_sin, quarter_hour_cos, day_sin, day_cos,
        # day_in_week_sin, day_in_week_cos

        # Extract features
        hour = x[:, :, 0].long()  # Extract 'hour'
        mins_position = x[:, :, 1].long()  # Extract 'quarter_hour'

        hour_sin = x[:, :, 4].unsqueeze(-1)  # Extract 'hour_sin'
        hour_cos = x[:, :, 5].unsqueeze(-1)  # Extract 'hour_cos'

        mins_position_sin = x[:, :, 6].unsqueeze(-1)  # Extract 'quarter_hour_sin'
        mins_position_cos = x[:, :, 7].unsqueeze(-1)  # Extract 'quarter_hour_cos'
...
if __name__ == "__main__":
    ...
    time_index = generate_time_series(start_time, end_time)
